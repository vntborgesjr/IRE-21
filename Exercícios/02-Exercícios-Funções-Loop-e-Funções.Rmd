...
estatistica_aplicacao <- strsplit(__, split = ":")
...
selecionar_primeiro_elemento <- function(__) {
  return(__[__])
}
```

- Aplique a a função `selecionar_primeiro_elemento()` nos elementos contidos em `estatistica_aplicacao_minusculo` e armazene os reusltados em um novo objeto chamdo `estatistica`.

```{r}
# Aplique a função selecionar_primeiro_elemento() sobre estatistica_aplicacao_minusculo
estatistica <- lapply(__, __)
...
```{r}
# Complete o código para finalizar a função
selecionar_segundo_elemento <- function(__) {
  return(__[__])
}
```
...
- Transforme a função `selecionar_primeiro_elemento()` em numa função que selecione o enésimo elemento de um objeto. Acrescente à função o argumento `posicao`. Salve essa função com o nome de `selecionar_enesimo_elemento()`. Em seguida, aplique-a sobre os elementos de `estatistica_aplicacao_minusculo` para obter o nome da estatística (`estatistica`) e sua aplicação (`aplicacao`). 

```{r}
# Reescreva a função adicionando um argumento que permita controlar a posição do elemento que será retornado
selecionar_enesimo_elemento <- function(__, __) {
  return(__[__])
}

# Selecione os primeiros elementos da lista estatistica_aplicacao_minusculo
estatistica <- lapply(__, 
                      __, 
                      __)
...
media_massa_piguins <- aggregate(penguins$__, 
          list(Especie = penguins$__),
          __, na.rm = TRUE)
```
...
- Agora, gere a tabela com as estatísticas descritivas usando a função `data.frame()` e os objetos contendo as estatísticas descritivas para as três espécies: `media_massa_piguins`, `dp_massa_piguins`, `max_massa_piguins`, `min_massa_piguins` e `n_massa_piguins`. Armazene o resultado em um novo objeto chamado `estatistica_descritiva_massa`.

```{r}
# Gere a tabela contendo a estatística descritiva da massa das três espécies de pinguins
estatistica_descritiva_massa <- data.frame(Especie = __$__, 
                                           Media = __$__,
                                           Desv_Pad = __$__,
                                           Max = __$__,
                                           Min = __$__,
...
```{r}
estatistica_descritiva <- function(__, __, na.rm = TRUE) {
  # Calcule a massa corporal média de cada espécie de pinguim
  media <- aggregate(__, 
          list(Especie = __),
          __, na.rm = na.rm)
...
estatistica_descritiva(__$__, __$__)
```
