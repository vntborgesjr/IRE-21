...
```{r}
# Carregar os dados
mypenguins_raw <- read_csv("mypenguins_raw.csv", col_names = TRUE, guess_max = 0) # não altere os argumentos
```
...
```{r}
mypenguins_clean1 <- mypenguins_raw %>% 
  select(date = `Date Egg`, species = `Species`, island = `Island`, culmen_len = `Culmen Length (mm)`, culmen_depth = `Culmen Depth (mm)`, flip_len = `Flipper Length (mm)`, body_mass = `Body Mass (g)`, sex = `Sex`, deltaN = `Delta 15 N (o/oo)`, deltaC = `Delta 13 C (o/oo)`)
```
...
As variáveis `date`, `culmen_len` e `body_mass` deveriam ser do tipo `date` e `double` (`dbl`) mas foram atribuídas como `character`. Também é necessário abreviar o nome das espécies de pinguim. Além disso, será conveniente que as variáveis `species`, `island` e `sex` sejam do tipo `factor`. Utilize a função `mutate()` do pacote `dplyr` para:
...
- Substitua o nome das espécies pelos seus apelidos `Adelia`, `Gentoo` e `Chinstrap` utilizando a função `str_replace_all()` do pacote `stringr`.

- Converter as variáveis `species`, `island` e `sex` para o tipo `factor`.

- Armazene as alterações realizadas no *data frame* em um novo objeto chamado `mypenguins_clean2`.
...
# efetuar alterações
mypenguins_clean2 <- mypenguins_clean1 %>% 
  mutate(date = ymd(date),
         species = as.factor(str_replace_all(mypenguins_clean1$species, coll(c("Adelie Penguin (Pygoscelis adeliae)" = "Adelie",                     "Chinstrap penguin (Pygoscelis antarctica)" = "Gentoo",               "Gentoo penguin (Pygoscelis papua)" = "Chinstrap")))),
         island = as.factor(island),
         culmen_len = as.numeric(culmen_len),
         culmen_depth = as.numeric(culmen_depth),
         flip_len = as.numeric(flip_len),
         body_mass = as.numeric(body_mass),
         sex = as.factor(sex),
         deltaN = as.numeric(deltaN),
...
- Faça a conversão dos tipos de dados ao carregá-lo e armazene no objeto `mypenguins_clean2`. **Não** realize a conversão da variável `Species` nessa etapa. Antes de convertê-la a fator será necessário alterar o nome das espécies. Essa tarefa será realizada na próxima etapa.

```{r}
mypenguins_clean2 <- read_csv("mypenguins_raw.csv",
                          col_types = cols(
                            Island = col_factor(),
                            `Date Egg` = col_date(),
...
mypenguins_clean2 <- mypenguins_clean2 %>% 
  select(date = `Date Egg`, species = `Species`, island = `Island`, culmen_len = `Culmen Length (mm)`, culmen_depth = `Culmen Depth (mm)`, flip_len = `Flipper Length (mm)`, body_mass = `Body Mass (g)`, sex = `Sex`, deltaN = `Delta 15 N (o/oo)`, deltaC = `Delta 13 C (o/oo)`) %>% 
  mutate(species = as.factor(str_replace_all(species, coll(c("Adelie Penguin (Pygoscelis adeliae)" = "Adelie",
...
Muitas vezes precisamos trabalhar apenas com um subconjunto das observações do *data frame*. O "verbo" `filter()` do pacote `dplyr` permite que obtenhamos apenas o subconjunto das observações que necessitamos. Com ele é possível flitrar subconjuntos de observações de uma ou mias variáveis ao mesmo tempo.
...
adelie <- mypenguins_clean2 %>% 
  filter(species == "Adelie")

# gerar data frame com as observações da espécie chinstrap
chinstrap <- mypenguins_clean2 %>% 
  filter(species == "Chinstrap")

# gerar data frame com as observações da espécie gentoo
gentoo <- mypenguins_clean2 %>% 
  filter(species == "Gentoo")
```

- Inspecione os objetos criados e visualize as 10 primeiras linhas usando as funções `head()` e `glimpse()`.
...
- Filtre o *data frame* `mypenguins_clean2` a partir das variáveis `island` e `species` e obtenha subconjunto das observações de cada espécies para cada ilha. Armazene os resultados nos objetos `adelie_biscoe`, `adelie_dream`, `adelie_torgersen`, `chinstrap_biscoe`, `chinstrap_dream`, `chinstrap_torgersen`, `gentoo_biscoe`, `gentoo_dream` e `gentoo_torgersen`.
...
chinstrap_desc <- mypenguins_clean2 %>% 
  filter(species == "Chinstrap") %>% 
  select(body_mass) %>% 
  arrange(desc(body_mass)) %>% 
  head(n = 1)
...
```{r}
# obtendo o subconjunto de observações da espécie Adelie para cada ilha
adelie_biscoe_havier <- mypenguins_clean2 %>% 
  filter(species == "Adelie", island == "Biscoe") %>% 
  select(body_mass) %>% 
  arrange(body_mass) %>% 
  head(n = 1)

adelie_dream_havier <- mypenguins_clean2 %>% 
  filter(species == "Adelie", island == "Dream") %>% 
  select(body_mass) %>% 
  arrange(body_mass) %>% 
  head(n = 1)

adelie_torgersen_havier <- mypenguins_clean2 %>% 
  filter(species == "Adelie", island == "Torgersen") %>% 
  select(body_mass) %>% 
  arrange(body_mass) %>% 
  head(n = 1)
...
# obtendo o subconjunto de observações da espécie Chinstrap para cada ilha
chinstrap_biscoe_havier <- mypenguins_clean2 %>% 
  filter(species == "Chinstrap", island == "Biscoe") %>% 
  select(body_mass) %>% 
  arrange(body_mass) %>% 
  head(n = 1)

chinstrap_dream_havier <- mypenguins_clean2 %>% 
  filter(species == "Chinstrap", island == "Dream") %>% 
  select(body_mass) %>% 
  arrange(body_mass) %>% 
  head(n = 1)

chinstrap_torgersen_havier <- mypenguins_clean2 %>% 
  filter(species == "Chinstrap", island == "Torgersen") %>% 
  select(body_mass) %>% 
  arrange(body_mass) %>% 
  head(n = 1)
...
# obtendo o subconjunto de observações da espécie Gentoo para cada ilha
gentoo_biscoe_havier <- mypenguins_clean2 %>% 
  filter(species == "Gentoo", island == "Biscoe") %>% 
  select(body_mass) %>% 
  arrange(body_mass) %>% 
  head(n = 1)

gentoo_dream_havier <- mypenguins_clean2 %>% 
  filter(species == "Gentoo", island == "Dream") %>% 
  select(body_mass) %>% 
  arrange(body_mass) %>% 
  head(n = 1)

gentoo_torgersen_havier <- mypenguins_clean2 %>% 
  filter(species == "Gentoo", island == "Torgersen") %>% 
  select(body_mass) %>% 
  arrange(body_mass) %>% 
  head(n = 1)
...
mypenguins_clean2 %>% 
  filter(species == "Adelie") %>% 
  mutate(ratio_culmen_depth_len = culmen_depth/culmen_len) %>% 
  select(species, ratio_culmen_depth_len) %>% 
  head(n = 1)
```
...
mypenguins_clean2 %>% 
  drop_na() %>% 
  summarise(mean_all = mean(body_mass))
```
...
mypenguins_clean2 %>% 
  filter(species == "Adelie") %>% 
  drop_na() %>% 
  summarise(mean_mass_adelie = mean(body_mass))
```
...
mypenguins_clean2 %>% 
  filter(species == "Chinstrap") %>% 
  drop_na() %>% 
  summarise(mean_mass_chinstrap = mean(body_mass))
```
...
mypenguins_clean2 %>% 
  filter(species == "Gentoo") %>% 
  drop_na() %>% 
  summarise(mean_mass_gentoo = mean(body_mass))
```
...
- Utilize a função `summarise()` para gerar um sumário estatístico das observações de massa dos pinguins, calculando a média (`mean()`), o desvio padrão (`sd()`), a variância (`var()`), o intervalo interquartil (`quantile()`) e o número de observações realizadas para cada espécie (`n()`). 
...
quantile(mypenguins_clean2$body_mass, na.rm = TRUE)
```
...
mypenguins_clean2 %>% 
  ...
  drop_na() %>% 
  summarise(mean_mass_adelie = mean(body_mass),
            sd_mass_adelie = sd(body_mass),
            min_mass_adelie = quantile(body_mass)[1],
            qrt2_mass_adelie = quantile(body_mass)[2],
            median_mass_adelie = quantile(body_mass)[3],
            qrt3_mass_adelie = quantile(body_mass)[4],
            max_mass_adelie = quantile(body_mass)[5],
            n_mass_adelie = n())
```
...
mypenguins_clean2 %>% 
  ...
  drop_na() %>% 
  summarise(mean_mass_chinstrap = mean(body_mass),
            sd_mass_chinstrap = sd(body_mass),
            min_mass_chinstrap = quantile(body_mass)[1],
            qrt2_mass_chinstrap = quantile(body_mass)[2],
            median_mass_chinstrap = quantile(body_mass)[3],
            qrt3_mass_chinstrap = quantile(body_mass)[4],
            max_mass_chinstrap = quantile(body_mass)[5],
            n_mass_chinstrap = n())
```
...
mypenguins_clean2 %>% 
  ...
  drop_na() %>% 
  summarise(mean_mass_gentoo = mean(body_mass),
            sd_mass_gentoo = sd(body_mass),
            min_mass_gentoo = quantile(body_mass)[1],
            qrt2_mass_gentoo = quantile(body_mass)[2],
            median_mass_gentoo = quantile(body_mass)[3],
            qrt3_mass_gentoo = quantile(body_mass)[4],
            max_mass_gentoo = quantile(body_mass)[5],
            n_mass_gentoo = n())
```
...
mypenguins_clean2 %>% 
  ...
  summarise(mean_mass = mean(body_mass),
            ...
            min_mass = quantile(body_mass)[1],
            qrt2_mass = quantile(body_mass)[2],
            median_mass = quantile(body_mass)[3],
            qrt3_mass = quantile(body_mass)[4],
            max_mass = quantile(body_mass)[5],
            n_mass = n())
